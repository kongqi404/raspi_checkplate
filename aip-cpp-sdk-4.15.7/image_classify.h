...
namespace aip {
    ...
    public:
            ...
            std::map<std::string, std::string>  options)
        {
            ...
            std::string url = _combination+mid+access_token;
            Json::Value result =
         ...
         * sceneConf 对特定服务，支持的个性化参数，若不填则使用默认设置
         */
        Json::Value combinationUrl(
            std::string const & imgUrl,
            std::vector<std::string> scenes,
            std::map<std::string, std::string>  options)
        {
            std::string access_token = this->getAccessToken();
         ...
         * @param image 二进制图像数据         * @param case_id 任务ID（通过case_id区分不同视频流，自拟，不同序列间不可重复）         * @param case_init 每个case的初始化信号，为true时对该case下的跟踪算法进行初始化，为false时重载该case的跟踪状态。当为false且读取不到相应case的信息时，直接重新初始化         * @param area 只统计进出该区域的车辆。逗号分隔，如‘x1,y1,x2,y2,x3,y3...xn,yn'，按顺序依次给出每个顶点的x、y坐标（默认尾点和首点相连），形成闭合多边形区域。
         * options 可选参数:
